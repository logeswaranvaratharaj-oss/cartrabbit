...
## 4. GETTING STARTED & SETUP COMMANDS

### Database Setup
1.  **Run Migrations**: Create all necessary tables (users, bookings, availability, etc.)
    ```bash
    php artisan migrate
    ```
2.  **Seed Data**: Populate the application with default settings and an admin user.
    ```bash
    php artisan db:seed
    ```
...
*   **Password**: `password`
*   **Default Settings**: Monday-Friday (9 AM - 5 PM) with 15/30/60m meeting options.
...
### Troubleshooting
*   If you see "Empty Slots", ensure you have run the seeder!
*   If you get connection errors, verify the `.env` file uses the correct `DB_` credentials for your local XAMPP/MySQL setup.
